...
query = "prepare catinsert( timestamp, timestamp, geometry" + \
        ", text" * len(prod_re) + \
        ") as insert into catalog (starttime, stoptime, footprint, " + \
...
for path in glob.iglob("/mnt/ifs-gis/ftp/*/modis/archive/*/*/?????/????"):
    ...
    starttime = gd.GetMetadataItem("RANGEBEGINNINGDATE") + " " + gd.GetMetadataItem("RANGEBEGINNINGTIME")
    stoptime  = gd.GetMetadataItem("RANGEENDINGDATE") + " " + gd.GetMetadataItem("RANGEENDINGTIME")
    ...
    fp = "POLYGON(( {north} {west}, {north} {east}, {south} {east}, {south} {west}, {north} {west} ))".format(**coord)
